...
if noise == 'no_adaptation':
    ...
    noise_imgs = torch.load(dir+'datasets/noiseMNIST/data/' + str(t_steps_train) + '_' + noise + '_' + dataset + '_imgs_' + contrast)
    noise_lbls = torch.load(dir+'datasets/noiseMNIST/data/' + str(t_steps_train) + '_' + noise + '_' + dataset + '_lbls_' + contrast)
dt = noiseMNIST_dataset(noise_imgs, noise_lbls)
print('Shape training set: ', noise_imgs.shape, ', ', noise_lbls.shape)

# best fit using grid search
tau1_init = torch.Tensor([0.034268])
tau2_init = torch.Tensor([0.540629])
sigma_init = torch.Tensor([0.006827])

# serve as placeholders (no training being performed for the current script)
train_alpha = False
train_beta = False
train_tau1 = False
train_tau2 = False
...
model_exp_decay = cnn_feedforward_exp_decay(train_alpha, train_beta, t_steps=t_steps)
model_exp_decay.load_state_dict(torch.load(dir+'weights/weights_feedforward_exp_decay_' + noise + '_' + contrast + '_' + param + '.pth'))    

model_div_norm = cnn_feedforward_div_norm(tau1_init, train_tau1, tau2_init, train_tau2, sigma_init, train_sigma, batchsiz=batchsiz, t_steps=t_steps, sample_rate=sample_rate)
model_div_norm.load_state_dict(torch.load(dir+'weights/weights_feedforward_div_norm_' + noise + '_' + contrast + '_' + param + '.pth'))    

print('Models loaded!')

# create stimulus timecourse
# extract adapter and adapter+target
adapter = noise_imgs[idx, start_train[0], :, :, :]
isi = noise_imgs[idx, 0, :, :, :]
target = noise_imgs[idx, start_train[1], :, :, :]
...
for t in range(t_steps):
    ...
    if t_steps_label[t] == 0:           
        ...
        imgs_seq.append(adapter)
    elif t_steps_label[t] == 2:                 # same
        imgs_seq.append(target)
...
if plot:
    ...
    fig2.savefig(dir+'visualizations/stimulus_timecourse', dpi=300)
    # plt.show()
    plt.close()
...
for i in range(len(layers)):
    if layers[i] in layers_plot:
        ...
        # plot stimulus
        axs[0].axvspan(start[0], start[0]+dur[0], color='grey', alpha=0.05)
        axs[0].axvspan(start[1], start[1]+dur[1], color='grey', alpha=0.05)
...
for i in range(len(layers_exp_decay)):
    if layers_exp_decay[i] in layers_plot_exp_decay:
        ...
        # plot stimulus
        axs[1].axvspan(start[0], start[0]+dur[0], color='grey', alpha=0.05)
        axs[1].axvspan(start[1], start[1]+dur[1], color='grey', alpha=0.05)
...
for i in range(len(layers_div_norm)):
    if layers_div_norm[i] in layers_plot_div_norm:
        ...
        # plot stimulus
        axs[2].axvspan(start[0], start[0]+dur[0], color='grey', alpha=0.05)
        axs[2].axvspan(start[1], start[1]+dur[1], color='grey', alpha=0.05)
...
# show plot
fig.savefig(dir+'visualizations/activations_readouts_generalization')
fig.savefig(dir+'visualizations/activations_readouts_generalization.svg', format='svg')
plt.show()
