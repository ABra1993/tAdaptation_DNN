...
# plot settings
fig, axs = plt.subplots(len(tempDynamics)*len(contrasts)+2, t_steps)#, figsize=(10, 2*len(dataset)))
    ...
    for a, (imgs, lbls) in enumerate(ldrTest):
        ...
        for iC, contrast in enumerate(contrasts):
            ...
            for itDyn, current_tempDynamics in enumerate(tempDynamics):
                ...
                # create sequence
                ax = sequence_test(imgs, None, contrast)
                ...
                for t in range(t_steps):
                    ...
                    if current_dataset != 'cifar':
                        axs[iC*4+iC, t].imshow(ax[t][0, :, :, :].permute(1, 2, 0), cmap='gray')
                    else:
                        axs[iC*4+iC, t].imshow(ax[t][0, :, :, :].permute(1, 2, 0))
                    ...
                    # plot activations
                    axs[iC*4+itDyn+iC+1, t].imshow(fmap.permute(1, 2, 0), cmap='viridis')

                    # adjust axes
                    axs[iC*4+itDyn+iC, t].axis('off')
                    axs[iC*4+itDyn+1+iC, t].axis('off')


# save figure
# plt.tight_layout()
plt.savefig(root + 'visualization/feature_maps/feature_map_' + task)
plt.savefig(root + 'visualization/feature_maps/feature_map_' + task + '.svg')
