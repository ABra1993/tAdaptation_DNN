...
df_trained = pd.read_csv('accu/grid_search/meta.txt', index_col=0, sep=' ', header=0)
print(df_trained)

# make selection
thrs = 0.6
df_select = df_trained[df_trained.acc > thrs]
# df_select = df_select.sort_values('acc')
df_select.reset_index(drop=True, inplace=True)
print(df_select)
print('Number of networks with accuracy above %i: %i' % (thrs*100, len(df_select)))
...
for i in range(len(df_select)):
    ...
    tau1_init = torch.Tensor([df_select.loc[i, 'tau1_init']])
    tau2_init = torch.Tensor([df_select.loc[i, 'tau2_init']])
    sigma_init = torch.Tensor([df_select.loc[i, 'sigma_init']])
    sample_rate = torch.Tensor([df_select.loc[i, 'sample_rate']])

    noise_prev = df_select.loc[i, 'noise']
    if noise_prev == 'same':
        noise = 'different'
    elif noise_prev == 'different':
        noise = 'same'

    df.loc[i, 'Init'] = df_select.loc[i, 'Init']
    ...
    df.to_csv('accu/grid_search3/meta.txt', header=True, sep=' ')
    print(df)
    ...
    if float(torch.mean(accuracies)) > 0.5:
        ...
        # save accuracies
        if adapt == 'exp_decay':
            torch.save(accuracies, 'accu/grid_search3/'+ str(i+1) + '_feedforward_' + adapt + '_' + noise + '_' + contrast)
        elif adapt == 'div_norm':
            torch.save(accuracies, 'accu/grid_search3/'+ str(i+1) + '_feedforward_' + adapt + '_' + noise + '_' + contrast)
        ...
        # save model
        torch.save(model.state_dict(), dir+'weights/grid_search3/weights_init_' + str(i) + '_run_' + str(j) + '.pth')
        ...
        # save figure
        plt.legend()
        plt.savefig('visualizations/grid_search3/' + str(i+1), dpi=300)
        plt.close()
